<?php
// file: manage_work.php
// 1. ส่วน Logic: ต้องอยู่บนสุด ก่อนเรียก Header เพราะมีการใช้ header() redirect
include_once 'db.php'; 
if (session_status() == PHP_SESSION_NONE) { session_start(); }

// ตรวจสอบสิทธิ์ (กันเหนียว เผื่อ header.php ยังไม่ทำงานในส่วนนี้)
if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'teacher') { header("Location: login.php"); exit(); }

$sweet_alert = "";

// --- Logic: Toggle Active Status (เปิด/ปิดสอบ) ---
if (isset($_GET['toggle_exam']) && isset($_GET['status'])) {
    ...
    header("Location: manage_work.php"); // Redirect ทันที
    exit();
}
...
if (isset($_POST['save_work'])) {
    ...
    if ($stmt->execute()) {
        $sweet_alert = "Swal.fire({
            icon: 'success', title: 'เพิ่มงานสำเร็จ!', text: 'บันทึกข้อมูลเรียบร้อยแล้ว',
            confirmButtonColor: '#ff007f', timer: 2000
...
if (isset($_POST['update_work'])) {
    ...
    if ($stmt->execute()) {
        $sweet_alert = "Swal.fire({
            icon: 'success', title: 'แก้ไขสำเร็จ!', text: 'ข้อมูลได้รับการอัปเดตแล้ว',
            confirmButtonColor: '#ff007f', timer: 2000
        }).then(() => { window.location = 'manage_work.php'; });";
    }
    $stmt->close();
...
// --- 2. เรียก Header (เริ่มแสดงผล HTML) ---
include 'header.php';
...
$works_by_course = [];
// Query ดึงงาน + PIN
$sql_works = "SELECT w.*, e.exam_id, e.is_active, e.access_pin, 
        ...
        <?php else: ?>
            ...
            <div class="work-item">
                ...
                <div class="item-right">
                    ...
                    <div class="stat-group" style="min-width: 60px; border-left: 1px solid #eee; padding-left: 15px;">
                        <?php if($has_exam_set): ?>
                            <span class="stat-val text-primary" style="letter-spacing: 1px;">
                                <?php echo htmlspecialchars($row['access_pin']); ?>
                            </span>
